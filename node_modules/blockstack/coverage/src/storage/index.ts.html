...
<a name='L657'></a><a href='#L657'>657</a></td><td class="line-coverage quiet"><span class="cline-any cline-neutral">&nbsp;</span>
 ...
 */
export function <span class="fstat-no" title="function not covered" >deleteFile(</span>path: string) {
<span class="cstat-no" title="statement not covered" >  Promise.reject(new Error(`Delete of ${path} not supported by gaia hubs`))</span>
}
&nbsp;
/**
...
export function getFile(
  ...
  const defaults = {
    ...
    verify: false,
    username: null,
    app: typeof window !== 'undefined' ? <span class="branch-0 cbranch-no" title="branch not covered" >window.location.origin </span>: undefined,
...
&nbsp;
  return fetch(`${hubConfig.server}/list-files/${hubConfig.address}`, fetchOptions)
    .then((response) =&gt; {
      ...
      if (httpStatus &gt;= 400) {
<span class="cstat-no" title="statement not covered" >        throw new Error(`listFiles failed with HTTP status ${httpStatus}`)</span>
      }
      return response.text()
    })
...
<span class="cstat-no" title="statement not covered" >          return Promise.resolve(fileCount + i)</span>
        }
      }
...
export async function listFiles(
  ...
  const userSession = caller || <span class="branch-1 cbranch-no" title="branch not covered" >new UserSession()</span>
